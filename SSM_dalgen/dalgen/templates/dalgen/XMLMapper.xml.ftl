...
<#list xmlMapper.table.columnList as column>
        <#if column.sqlName?upper_case =="ID"><id column="${column.sqlName}" property="${column.javaName}" jdbcType="${column.sqlType}" javaType="${fun.javaTypeTrun(column.javaType)}" ${fun.typeHandler(column)}/><#else><result column="${column.sqlName}" property="${column.javaName}" jdbcType="${column.sqlType}" javaType="${fun.javaTypeTrun(column.javaType)}" ${fun.typeHandler(column)}/></#if>
</#list>
    </resultMap>
</#if>
...
<#list xmlMapper.resultMaps as resultMap>
    ...
    <#list resultMap.columnList as column>
        <#if column.sqlName?upper_case =="ID"><id column="${column.sqlName}" property="${column.javaName}"  javaType="${fun.javaTypeTrun(column.javaType)}"/><#else><result column="${column.sqlName}" property="${column.javaName}"  javaType="${fun.javaTypeTrun(column.javaType)}"  ${fun.typeHandler(column)}/></#if>
    </#list>
...
${operation.cdata!}
    ...
    <#else><#--非分页-->
    <!--${operation.remark!operation.name}  自动生成，请勿修改-->
