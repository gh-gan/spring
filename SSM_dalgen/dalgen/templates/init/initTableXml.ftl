...
<table sqlname="${table.sqlName?lower_case}" physicalName="${table.physicalName?lower_case}"<#if table.remark??> remark="${table.remark!}"</#if>>
    ...
    <operation name="insert" paramtype="object" remark="插入表:${table.sqlName?lower_case}" autoGen="true">
        <@m.insertMethod table/>
    </operation>

    <!-- foreach 可以自定义类型，paramtype="primitive" foreach->javatype="自己书写的类"  -->
    <!-- 只有一个参数且为List时必须将参数命名为list -->
    <operation name="insertBatch" paramtype="objectList" remark="批量插入表:${table.sqlName?lower_case}"  autoGen="true">
        <@m.insertBatchMethod table/>
    </operation>

<#if table.primaryKeys??>
    <!-- 不推荐使用全 update 有需要自己打开
    <operation name="update" paramtype="object" remark="更新表:${table.sqlName?lower_case}"  autoGen="true">
        <![CDATA[
